<template>
    <button @click="download" :disabled="isLoading" class="download-button">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
            <path d="M352 96C352 78.3 337.7 64 320 64C302.3 64 288 78.3 288 96L288 306.7L246.6 265.3C234.1 252.8 213.8 252.8 201.3 265.3C188.8 277.8 188.8 298.1 201.3 310.6L297.3 406.6C309.8 419.1 330.1 419.1 342.6 406.6L438.6 310.6C451.1 298.1 451.1 277.8 438.6 265.3C426.1 252.8 405.8 252.8 393.3 265.3L352 306.7L352 96zM160 384C124.7 384 96 412.7 96 448L96 480C96 515.3 124.7 544 160 544L480 544C515.3 544 544 515.3 544 480L544 448C544 412.7 515.3 384 480 384L433.1 384L376.5 440.6C345.3 471.8 294.6 471.8 263.4 440.6L206.9 384L160 384zM464 440C477.3 440 488 450.7 488 464C488 477.3 477.3 488 464 488C450.7 488 440 477.3 440 464C440 450.7 450.7 440 464 440z"/>
        </svg>
        <span v-if="isLoading">Crunching data</span>
        <span v-else>Download</span>
    </button>
</template>

<script setup>
    import { ref } from 'vue';
    import { getTrackTitle, getProvider, formatProviderUrl, formatTimestampForFilename } from '../utils';

    const props = defineProps({
        filename: {
            type: String,
            required: true
        },
        url: {
            type: String,
            required: true,
        },
    });

    const isLoading = ref(false);
    const content = ref(null);

    const formatContent = (data = []) => {
        const groups = [
            'YouTube',
            'SoundCloud',
            'Bandcamp',
            'Vimeo',
            'YouTube Flagged',
            'SoundCloud Flagged',
            'Bandcamp Flagged',
            'Vimeo Flagged',
            'Unknown',
        ];

        const grouped = Object.groupBy(data, item => {
            const provider = getProvider(item);
            return (item.isFlagged && provider !== 'Unknown') ? `${provider} Flagged` : provider;
        });

        return groups.map(group => {
            const items = grouped[group] ?? [];
            const body = items.length > 0
                ? items.map(item => `${formatProviderUrl(item)}\t${getTrackTitle(item)}`).join('\n')
                : '—';
            return `[${group}]\n${body}`;
        }).join('\n\n');
    };

    const fetchContent = async () => {
        if(content.value) return;

        isLoading.value = true;

        const downloadResponse = await fetch(props.url);
        const downloadJson = await downloadResponse.json();

        content.value = formatContent(downloadJson.data);

        isLoading.value = false;
    };

    const download = async () => {
        await fetchContent();

        const blob = new Blob([content.value], { type: 'text/plain;charset=utf-8' });
        const href = URL.createObjectURL(blob);

        Object.assign(document.createElement('a'), {
            href,
            download: `${props.filename}_${formatTimestampForFilename()}.txt`,
        }).click();

        requestAnimationFrame(() => URL.revokeObjectURL(href));
    };
</script>

<style>
    .download-button {
        display: flex;
        gap: .25rem;
        background-color: light-dark(var(--color-accent-dark), var(--color-accent-light));
        color: light-dark(var(--color-lightest), var(--color-darkest));
        padding: .25rem .5rem;
    }

    .download-button svg {
        width: 1rem;
    }
</style>
